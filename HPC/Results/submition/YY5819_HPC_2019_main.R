...
question_12 <- function()  {
  ...
  legend(120, 90,legend=c("Max initial richness", "Min initial richness"), col=c("blue", "red"), lty=1:2)
...
process_cluster_results <- function (iter=100){
  ...
  #print(final)
  save(final, file="YY5819_cluster_results.rda")
  return(final)
...
Challenge_A <- function(duration=200,v=0.1,size=100,alpha=1-0.972,iter=30) {
  ...
  segments(c(1:n),CI$minBtm,c(1:n),CI$minTop,col="green")   #confidence interval
...
calRichness <- function(initial, v, rep, duration){
  ...
  for (i in 1:(rep)){ 
    ...
    richness=sum_vect(richness, timeS)
    sumRich=sum_vect(sumRich, (timeS)^2)
  }
  out=list(richness, sumRich)
  return(out)
}
Challenge_B <- function(pop=100, v=0.1, iter=10, duration=200){
  ...
  for (i in 1:pop){
    if (pop %% i == 0){ 
      ...
      rich_mean=sumRich[[1]]/iter
      allRichness=c(allRichness, i) 
...
Challenge_F <- function(){
  # clear any existing graphs and plot your graph within the R window
  fern_c <- function(start_position, direction, length, dir, limit, col)  {
    if (length > limit){
      angle = direction
      y = cos(angle)*length
      x = sin(angle)*length
      segments(start_position[1], start_position[2], x+start_position[1], y+start_position[2], col = col)
      P1 = c(x+start_position[1],y+start_position[2])
      if (dir==-1){
        P2 = fern_c(c(P1), direction-(pi/4), 0.38*length, dir=-1, limit, col)
        P3 = fern_c(c(P1), direction, 0.87*length, dir=1, limit, col) 
      }
      else if (dir==1){
        P2 = fern_c(c(P1), direction+(pi/4), 0.38*length, dir=1, limit, col)
        P3 = fern_c(c(P1), direction, 0.87*length, dir=-1, limit, col)
  ...
  tree_c <- function(start_position, direction, length, limit, col)  {
    if (length > limit){
      angle = direction
      y = cos(angle)*length
      x = sin(angle)*length
      segments(start_position[1], start_position[2], x+start_position[1], y+start_position[2], col = col)
      P1 = c(x+start_position[1],y+start_position[2])
      P2 = tree_c(c(P1), direction+(pi/4), 0.65*length, limit, col)
      P3 = tree_c(c(P1), direction-(pi/4), 0.65*length, limit, col)
  ...
  par(mfrow=c(2,2))
  plot(0:10,0:10, type = "n")
  fern_c(c(5,1), 2*pi, 1, -1, 0.005, "green")
  fern_c(c(5,1), 2*pi, 1, 1, 0.009, "blue") 
  fern_c(c(5,1), 2*pi, 1, -1, 0.08, "yellow")  
  fern_c(c(5,1), 2*pi, 1, 1, 0.05, "red")
  plot(0:10,0:10, type = "n")
  fern_c(c(2,1), pi/4, .5, 1, 0.02, "blue")
  plot(0:10,0:10, type = "n")
  tree_c(c(5,1), 2*pi, 1, 0.01, "green")
  tree_c(c(5,1), 2*pi, 1, 0.1, "red")
  plot(0:10,0:10, type = "n")
  tree_c(c(5,1), -pi/4, 2, 0.01, "blue")
  tree_c(c(5,1), -pi/4, 2, 0.2, "yellow")
  return("the smaller the limit(threshole), the longer it takes to plot the graph")
}
